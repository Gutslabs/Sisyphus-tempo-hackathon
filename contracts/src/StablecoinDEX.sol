...
contract StablecoinDEX is IStablecoinDEX {
        ...
        returns (uint128 amountIn)
    {
        ...
        if (baseForQuote) {
            ...
            while (remainingOut > 0) {
                ...
                // Fill the order
                uint128 nextOrder = _fillOrder(orderId, fillAmount);
                amountIn += fillAmount;

                if (nextOrder == 0) {
                    if (remainingOut > 0) {
                        // Move to next tick
                        (currentTick, ) = nextInitializedBidTick(key, currentTick);
                        if (currentTick == type(int16).min) {
                            revert IStablecoinDEX.InsufficientLiquidity();
                        }
            ...
            // Spending quote for base (Ask side)
            ...
            while (remainingOut > 0) {
                ...
                // Round UP quote spent
                amountIn +=
                    uint128((uint256(fillAmount) * uint256(price) + PRICE_SCALE - 1) / PRICE_SCALE);

                if (nextOrder == 0) {
                    if (remainingOut > 0) {
                        (currentTick, ) = nextInitializedAskTick(key, currentTick);
                        if (currentTick == type(int16).max) {
                            revert IStablecoinDEX.InsufficientLiquidity();
                        }
    ...
    /// @notice Helper to find the next initialized ask tick
    ...
    /// @notice Helper to find the next initialized bid tick
