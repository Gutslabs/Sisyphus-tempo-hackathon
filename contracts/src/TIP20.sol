...
contract TIP20 is ITIP20, TIP20RolesAuth {
    ...
    constructor(
        ...
        hasRole[admin][ISSUER_ROLE] = true;        // Grant issuer role to allow minting/burning
        hasRole[admin][PAUSE_ROLE] = true;         // Grant pause role
        hasRole[admin][UNPAUSE_ROLE] = true;       // Grant unpause role

        emit RoleMembershipUpdated(DEFAULT_ADMIN_ROLE, admin, sender, true);
        emit RoleMembershipUpdated(ISSUER_ROLE, admin, sender, true);
        emit RoleMembershipUpdated(PAUSE_ROLE, admin, sender, true);
        emit RoleMembershipUpdated(UNPAUSE_ROLE, admin, sender, true);
    }
