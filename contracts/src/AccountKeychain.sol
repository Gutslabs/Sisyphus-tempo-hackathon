...
contract AccountKeychain is IAccountKeychain {
    ...
    /// @dev Transient storage slot for the transaction key
    /// Set by the protocol during transaction validation to indicate which key signed the tx
    bytes32 private constant TRANSACTION_KEY_SLOT = keccak256("AccountKeychain.transactionKey");
    ...
    function _requireRootKey() internal view {
        address key;
        bytes32 slot = TRANSACTION_KEY_SLOT;
        assembly {
            key := tload(slot)
        }
        if (key != address(0)) {
